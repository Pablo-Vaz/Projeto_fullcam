...
async def listar_camera(camera_id: int, db: AsyncSession = Depends(get_db)):
...
async def criar_camera(camera: CameraCriarAtt, db: AsyncSession = Depends(get_db)):
    new_cam = Camera(
        nome = camera.nome,
...
async def atualizar_camera(camera_id: int,dados: CameraCriarAtt, db: AsyncSession = Depends(get_db)):
...
async def atualizar_status(camera_id: int, mudar: CameraAttStatus, db: AsyncSession = Depends(get_db)):
    camera = await db.get(Camera, camera_id)
    if not camera:
        raise HTTPException(status_code=404, detail='Camera não encontrada')
